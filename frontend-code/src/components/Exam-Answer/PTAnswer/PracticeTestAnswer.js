    ...
    const fetchData = async () => {
      try {
        ...
        if (structureResponse.status === 200) {
          ...
          // Sort sections by exam_name using natural string comparison
          // This correctly handles all naming patterns: "2.1" < "2.2" < "2.10", "Section 1" < "Section 2", etc.
        ...
        if (response.status === 200) {
          ...
          const blocksToProcess = sectionOrder.length > 0 
            ...
            : [...correctAnswersByType].sort((a, b) => 
                (a.block_id || 0) - (b.block_id || 0)
              );

          // When the structure API failed (sectionOrder empty), block_id order
          // may not match the logical section order.  Fetch each block's details
          // to obtain exam_name, then re-sort so tabs appear correctly.
          let fetchedBlockDetails = {};
          if (sectionOrder.length === 0 && blocksToProcess.length > 0) {
            try {
              const blockDetailPromises = blocksToProcess.map(block =>
                ajaxCall(
                  `/exam/block/${block.block_id}/`,
                  {
                    headers: {
                      Accept: "application/json",
                      "Content-Type": "application/json",
              ...
              const blockDetailResponses = await Promise.all(blockDetailPromises);

              blockDetailResponses.forEach((resp, idx) => {
                if (resp.status === 200) {
                  const blockId = blocksToProcess[idx].block_id;
                  fetchedBlockDetails[blockId] = resp.data;
                  blocksToProcess[idx].sectionName = resp.data.exam_name;
                }
              });

              // Re-sort by exam_name using natural string comparison
              ...
              console.warn('Failed to fetch block details for ordering:', err);
            }
          }
          ...
          // Create block to section map for later use (View Exam needs this)
          const blockMap = {};
          if (sectionOrder.length > 0) {
            ...
            // Use the block details we fetched in the fallback path
            Object.entries(fetchedBlockDetails).forEach(([blockId, data]) => {
              blockMap[parseInt(blockId)] = data;
            });
          }
