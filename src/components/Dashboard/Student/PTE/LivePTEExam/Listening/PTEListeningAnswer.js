...
const PTEListeningAnswer = () => {
  ...
  const renderQuestionWithAnswers = () => {
    ...
    if (blockData.sub_category === "LFIB") {
      const studentAnswersList = studentAnswers[0]?.studentAnswers || [];
      let modifiedHtml = blockData.question_other;

      const tempDiv = document.createElement("div");
      tempDiv.innerHTML = modifiedHtml;
      const inputElements = tempDiv.querySelectorAll("input[type='text']");

      inputElements.forEach((input, index) => {
        const questionNumber =
          parseInt(input.id.replace("blank", "")) || index + 1;
    ...
    if (
      ...
      radioInputs.forEach((radio) => {
        ...
        const optionElement =
          radio.closest("p") || radio.closest("div") || radio.closest("span"); // Get the parent p, div, or span of the radio
    ...
    <div>
      ...
      >
        {isLoading ? (
          ...
          <div>
            {blockData?.audio_transcript && (
              <div className="mb-2">
                <h5>Audio Transcript</h5>
                <div className="border p-2">
                  <p
                    dangerouslySetInnerHTML={{
                      __html: blockData.audio_transcript,
                    }}
                  />
                </div>
              </div>
