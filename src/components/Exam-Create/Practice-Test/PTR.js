...
const reducerPTR = (state, action) => {
  if (action.type === "reset") {
    return action.payload || intialPTR;
  }
...
const PTR = () => {
  ...
  const [createPTR, dispatchPTR] = useReducer(
    reducerPTR,
    intialPTR
  );
  ...
  const getExams = async () => {
    try {
      ...
      if (response.status === 200) {
        const reading = response?.data?.filter(
  ...
  const createPTReading = async (e) => {
    e.preventDefault();
    if (!validateForm()) return;
    try {
      const data = {
        Name: createPTR.Name,
        exam_test: createPTR.exam_test,
        Reading: createPTR.Reading,
      };
      const response = await ajaxCall(
        "/moduleListView/",
        {
          ...
          <div className="row">
            ...
            >
              ...
              <div className="create__course__bottom__button">
                ...
                <button className="default__button" onClick={createPTReading}>
                  Create PT
                </button>
              </div>
            </div>
