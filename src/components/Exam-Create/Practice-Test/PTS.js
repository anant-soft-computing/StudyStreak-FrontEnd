...
const reducerPTS = (state, action) => {
  if (action.type === "reset") {
    return action.payload || intialPTS;
  }
...
const PTS = () => {
  ...
  const [createPTS, dispatchPTS] = useReducer(
    reducerPTS,
    intialPTS
  );
  ...
  const getExams = async () => {
    try {
      ...
      if (response.status === 200) {
        const speaking = response?.data?.filter(
          (exam) => exam.exam_type === "Speaking"
        );
        setExamList(speaking);
      } else {
        console.log("error");
      }
    } catch (error) {
  ...
  const validateForm = () => {
    ...
    if (!createPTS.Speaking.length > 0) {
  ...
  const createPTSpeaking = async (e) => {
    e.preventDefault();
    if (!validateForm()) return;
    try {
      const data = {
        Name: createPTS.Name,
        exam_test: createPTS.exam_test,
        Speaking: createPTS.Speaking,
      };
      const response = await ajaxCall(
        "/moduleListView/",
        {
          ...
          <div className="row">
            ...
            >
              ...
              <div className="create__course__bottom__button">
                ...
                <button className="default__button" onClick={createPTSpeaking}>
                  Create PT
                </button>
              </div>
            </div>
